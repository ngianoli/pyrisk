...
class HrlAI(AI):
    """
    HrlAI: an Hierachical reinforcement learnig AI that plays with a policy loaded
    and that send information for future training.
    """
    ...
    # to give rewards
    def event(self, msg):
        # strategy 2 : reward at the end + reward for conquering an area
        statement = msg[0]

        if statement == "conquer":
            attacker = msg[1].name
            if self.player.name == attacker:
                conquered_terr = msg[4]
                ...
                if conquered_terr.area in self.player.areas:
                    ...
                    self.rewards_data += [0]*N
                    self.rewards_data[-1] = reward


        elif statement == "victory":
            winner = msg[1].name
            N = len(self.action_data) - len(self.rewards_data)
            self.rewards_data += [0]*N
            if self.player.name == winner:
                self.rewards_data[-1] = 100
            else:
                self.rewards_data[-1] = -20
